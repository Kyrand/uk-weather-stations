...
YEAR, MONTH, TMAX, TMIN, RAIN, SUN = 0, 1, 2, 3, 5, 6
def process_weather_stations():

    mc = pymongo.MongoClient()
    coll = mc.viz.uk_weather_stations 
    coll.drop()
    
    for fname in glob('data/*.txt'):
        data = {}
        data['id'] = fname.split('/')[1].split('.')[0]
        f = open(fname)
        title = f.readline()
        data['title'] = title.strip()
        line = f.readline()#.replace(',', '').split() 
        
        e = int(re.findall(r"\D(\d{1,6})E\D", " "+line+" ")[0].rjust(4, '0').ljust(6, '0'))
        n = int(re.findall(r"\D(\d{1,6})N\D", " "+line+" ")[0].rjust(4, '0').ljust(6, '0'))
        # print e, n
        lat, lng = OSGB36toWGS84(e, n)
        # print 'lat: %f, long: %f'%(lat, lng)
        data['coords'] = (lat, lng)
        
        while not f.readline().strip().startswith('degC'):
            pass
        ...
        for l in f:
            l = l.strip().split()
            try:
                ldata.append({'id': data['id'], 'coords': data['coords'],  'date': '%s:01:%s'%(l[MONTH].rjust(2, '0'), l[YEAR]), 'tmax': l[TMAX], 'tmin': l[TMIN], 'rain': l[RAIN], 'sun': l[SUN]})
            except IndexError:
                logger.error('Malformed data for %s: %s'%(title, repr(l)))
